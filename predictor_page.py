...
def render_predictor(model, tokenizer, device, gradcam_obj, fetch_surprise_video, run_prediction, render_results, append_prediction, download_thumbnail, fmt, CATEGORIES, accuracy_info):
        ...
        with col_out:
            ...
            if run_manual:
                if not title.strip(): 
                    ...
                    st.warning("Please upload a thumbnail.")
                else:
                    with st.spinner("Analysing..."):
                        ids, mask, img_t, pred_vpd = run_prediction(model, tokenizer, device, title, pil_manual)
                        
                    render_results(
                        model, tokenizer, device, gradcam_obj, ids, mask, img_t, pil_manual, pred_vpd, title=title
                    )
            ...
            if err:
                ...
                if not pil_s:
                    ...
                    with c2:
                        ...
                        render_results(
                            model, tokenizer, device, gradcam_obj, ids, mask, img_t, pil_s, pred_vpd,
                            title=video_info['title'], actual_vpd=video_info['actual_vpd'], video_info=video_info
                        )